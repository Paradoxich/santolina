...
1. ~~**`Seaside petunia`'s live hero is the wrong species**~~ — **DONE later the same day, [PR #138](https://github.com/Paradoxich/santolina/pull/138).** It is on the labelled Commons photo of the true species (Magnus Manske, CC BY-SA 3.0), and `is_curated` flipped to true in the same write because the row was held on its image alone. **So this session's tally is 8 of 16 approved, 8 held** — the 7/9 figures elsewhere in this entry were true when written and are one row stale.
2. **Three holds still need a hero pointed at a SPECIFIC candidate, and `scripts/set-plant-hero.ts` is now the way to do it** (also PR #138). It refuses any URL that is not already one of the row's candidates, which is the safety property: a candidate has been probed, measured, licence-checked and attributed, so the script cannot introduce an unvetted URL, a non-commercial licence, or a host `next/image` will refuse. `--why` is mandatory and lands in `image_pick_reason`, because the write overrides an automated judgment.
