 ...
 * no queries, no migrations. Public via a temporary entry in isPublicPath so
 * it opens without a session; it sits outside /design-system because that
 * layout clamps to 960px and page width is part of what is being judged.
 *
...
/**
 * The open product question this preview surfaced: the dashboard caps its
 * content at 1032px while the app shell hands it the whole viewport minus the
 * sidebar and a 48px right margin. On a wide display that leaves several
 * hundred px unused. Changing it is a decision about the dashboard too, not
 * just this page, so both are shown rather than one picked.
 */
...
> = {
  product: {
    ...
    note: 'What the dashboard uses today. Same cap, same leftover space on a wide display.',
  },
  wide: {
    label: 'Wider cap (1280)',
    className: 'max-w-[1280px]',
    note: 'Cards breathe without the grid losing its proportions. Would need the dashboard raised to match.',
  },
  fill: {
    label: 'Fill available',
    className: 'max-w-none',
    note: 'No cap. Watch the 3-up row: cards get wide enough that their content strands at the left edge.',
  },
}
...
export default function PlantPreviewPage() {
  ...
  return (
    // Mimics the real app shell (AppSidebar offset + mr-12) so the width the
    // cards get here is the width they would get in the product. The docs
    // layout under /design-system clamps to 960px, which made an earlier
    // version of this preview 72px narrower than the page it is previewing.
        ...
        <div className={`${WIDTHS[width].className} pb-16 pt-8 md:pt-12`}>
          ...
          <div className="mb-8 flex flex-col gap-item-gap rounded-md border border-divider-subtle bg-surface-subtle p-card-padding">
            <p className="text-label text-muted">
              Preview only. Sample data, frozen to 29 July 2026. Not wired to
              the database.
            </p>
