...
import { STYLE_TAGS, STYLE_DISPLAY_NAMES } from '@/lib/style-tags'
import { stylePlantCounts } from '@/lib/style-availability.generated'
...
/**
 * Derived, not hand-written — the styles the catalog can actually fill.
 *
 * The vocabulary lives in lib/style-tags.ts and is larger than this on
 * purpose: a style is judged against from the day it is defined, and appears
 * here the round its population clears STYLE_FILTER_FLOOR. Counts come from
 * lib/style-availability.generated.ts, regenerated by `pnpm catalog:state`.
 *
 * Ordered by frequency, like TYPE_OPTIONS, so the common choices lead the row.
 */
export const STYLE_OPTIONS: FilterOption[] = STYLE_TAGS.filter(
  (t) => stylePlantCounts[t] >= STYLE_FILTER_FLOOR
)
  .sort((a, b) => stylePlantCounts[b] - stylePlantCounts[a])
  .map((t) => ({ value: t, label: STYLE_DISPLAY_NAMES[t] }))
