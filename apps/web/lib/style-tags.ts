...
export const STYLE_TAG_PROMPT = `- style_tags: Subset of exactly ${JSON.stringify(
  ...
  - "sensory": grown for scent or touch first — perfume near paths and seats, aromatic or tactile foliage. Jasmine, daphne, lemon verbena, night-scented stock, philadelphus, scented-leaf pelargoniums, lamb's ears. Flower beauty alone does not qualify; the scent/touch must be the reason it's planted.
  ...
  Be selective. Most plants earn one tag; a good number earn none — an empty array is a valid answer. Two tags is uncommon and means the plant is genuinely central to both traditions. Three is rare. Membership in one style is never evidence for membership in a neighbouring one — decide each tag independently against its own bar.`
