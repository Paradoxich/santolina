...
async function main() {
  ...
  const runOptions = {
    step: 'apply-image-confirmations',
    // All four move in ONE writePlant statement per row, so none is the trap-28
    // shape where a stamp lands on a subset of the counted rows.
    writeSet: [
    ...
    // image_verified_at is SET here, never cleared, so it witnesses itself. The
    // other three are value columns and cannot be compared to an instant.
    // `is_curated` is settled by writePlant's `claim`, and it is declared
    // because this run really does write it — unlike a trigger cascade, which
    // would be claiming authorship of something the database did.
    evidence: [
      {
        kind: 'stamp',
    ...
    scope: `${ids.length} reviewer confirmation(s) from ${file}`,
    // A reviewer looked at a photograph. There is no model and no prompt; the
    // confirmations file IS the recipe, and its content hash identifies the
    // batch of verdicts this run applied.
    recipe: {
      model: 'human',
      template: text,
      ingredients: {},
