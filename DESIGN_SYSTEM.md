...
source scan with no secrets, unlike the main-only database jobs). Three checks:
**A** no token value appears in prose, **B** no token re-types another's
channels, **C** the `/design-system` list and `index.css` cover each other in
both directions. `token-consumers.generated.ts` is generated by walking the real
...
**Historical token values stay legal behind an explicit marker.** The 2026-07-07
audit snapshot keeps its record of deleted ramps via
`<!-- tokens:historical: reason -->`. The "Changes since this audit" drift log
below it does **not** get the exemption. Explicit rather than inferred.
...
- **The login placeholder stays on `text-faint`**, knowing it measures ~2.1:1
