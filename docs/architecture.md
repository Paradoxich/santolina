...
**Cite a section by its anchor, never by its number.** Every section carries a
named `<a id="...">` immediately above its heading, and that slug is the section's
address: `docs/architecture.md#safe-upsert` from code, `[the safe Trefle
upsert](#safe-upsert)` from prose. Numbers are display order and will change when
this file is regrouped; anchors will not. `pnpm docs:links` fails on a reference
that resolves to nothing and on any new `§N` citation. Numbers cited in artifacts
that cannot be edited — applied migrations, archived round reports — are mapped in
[the appendix](#appendix-retired-numbers).
...
> **Two parts of this section were reversed within two days — read the amendment at the end before trusting the routing or drawer mechanics below.** The species-vs-owned-instance split the section is built on is unchanged and still correct.
...
### Amendment, July 29 2026: routing went dynamic, and the diary drawer is gone

**"Routing deliberately stays flat" above is no longer true, and was reversed deliberately rather than drifted into.** Plant detail is a real path segment — `app/(app)/plants/[plantId]` (PR #129) — not `?plant=`. The reason is the thing the flat convention could not do: detail and its notes need to **share a route tree**, so moving between them does not remount through the list. `/plants/[id]/notes` is the notes view, and it is why the app now has its second dynamic segment. The original argument (don't introduce the app's first `[id]` for one screen) was sound when written; it stopped being sound once a second screen hung off the same record.

**The diary drawer is deleted, not relocated** (PR #133). `StorySection` moved onto the plant page and the notes list became its own route, so the drawer described above no longer exists in any form. What survives unchanged is the rule it existed to enforce: **the Story has exactly one home, on the thing the user owns**, and other surfaces only link to it.
...
## 33. One content width across the app
...
## 34. The plant you own is a dashboard for that plant
...
## 35. Explore: search results are ranked, and the query stopped over-fetching
