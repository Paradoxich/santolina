...
**Review before applying, always.** Two of the failure modes above — the missing establishment marker and the higher-rank match — produce *confident, plausible, badly wrong* answers rather than errors, and both were found by reading a report, not by anything failing. That is the standing reason this script defaults to report-only, and why `--apply` should follow a human read of the diff rather than replace it.
