...
## 16. Care tips: reusing `seasonal_rhythm`, no new AI or API calls

**Decision:** the Dashboard's Care Tips card (`getCareTips()` in `apps/web/lib/care-tips.ts`) is built entirely from data that already exists — each curated plant's `seasonal_rhythm` jsonb column (6 keys: `early_spring`, `late_spring`, `summer`, `late_summer`, `autumn`, `winter`, populated by the AI curation pass, see §6). No new AI generation, no new external API, no new DB column. This is the same "derive at render time" philosophy as bloom status (§10).

**Season derivation:** there was no existing month→season mapping onto this 6-value vocabulary anywhere in the codebase — `lib/bloom-status.ts` derives a 5-value bloom status from `bloom_months`, and `lib/trefle.ts` has a separate, unexported 4-value month→season map (`spring`/`summer`/`autumn`/`winter`) used only at Trefle-ingestion time to set `peak_season`. Neither fits. `getCurrentSeason()` adds a standalone month→season lookup for a Mediterranean/European climate: Dec–Feb → `winter`, Mar–Apr → `early_spring`, May–Jun → `late_spring`, Jul–Aug → `summer`, Sep → `late_summer`, Oct–Nov → `autumn`.

**Selection and prioritization:** for each palette plant with status `planted` or `planned` (not `considering`), `getCareTips()` reads `plant.seasonal_rhythm[currentSeason]`; plants missing that key are skipped rather than erroring. Capped at 5 tips. When there are more than 5 candidates, plants currently `blooming` or `pre-bloom` (via the existing `getBloomStatus()`, §10) are prioritized to the front of the list — a plant actively flowering right now is more actionable than one just sitting dormant — with the remaining slots filled in `listPalette()`'s existing order (most recently added first).
