...
- **The demo purge is the one caller that has to record what it failed to remove.** The four above delete their own rows and can afford to shrug at a storage error. `purgeExpiredDemoUsers` cannot: `deleteUser` cascades away the very `diary_entries` rows holding `photo_urls`, so a swallowed error leaves objects that nothing points at. It still does not block the account deletion — Storage and Postgres cannot be deleted atomically, and aborting would queue up demo accounts that never expire behind one bad object — it copies the paths onto `PurgeResult.orphanedPhotos` first, and both the cron route and the CLI print them. That is why `removeDiaryPhotos` returns an outcome rather than `void`.
