...
**Do not read a pairing failure on a closed round as a regression, and do not waive them one by one.** The real fix is a timestamp on `plant_combinations`, which is a schema change nobody has needed yet.
