...
- [ ] **Model sun as "best" + "tolerated" (post-test)** — the root fix for the recurring narrowing: two fields (thrives-in set + tolerates set) instead of one flat `sun_requirements` array, so the app can say "loves full sun, also fine in part shade" and the thrive-vs-tolerate ambiguity that the two AI passes keep disagreeing over stops existing. Schema + UI change; the widening step above is the interim backstop. **— Ana + Fable (design) → Sonnet/Opus (build)**
- [ ] **Editorial review of the cross-check's remaining flags** — 7 `plant_type` classifications (subshrub/tuber/succulent edge cases: Thymus, Euphorbia characias, Cyclamen, Eranthis, Sedum spectabile, Rudbeckia hirta, Eschscholzia), one hardiness value (Eranthis hyemalis max 9 vs. 7), and one sun contradiction (`Berberis aquifolium` stored `[full_sun]`, likely should be `[partial_sun, shade]` — a shade shrub). Judgment calls, intentionally unapplied. Nice-to-have, not a launch blocker.
